...
fn execute_locker_hook(
    ...
    match msg {
        ...
        LockerHookMsg::OnExtend {
            ...
            let stake = USER_STAKES.may_load(deps.storage, locker_id)?;
        ...
        LockerHookMsg::OnUnlock {
            ...
            if stake.is_none() {
                return Ok(Response::new()
                    ...
                    .add_attribute("sub_action", "on_unlock_skipped")
                    .add_attribute("reason", "stake_not_found"));
            }
...
fn execute_register_stake(
    ...
    if USER_STAKES.has(deps.storage, locker_id) {
    ...
    // Define local types for querying LP Locker
    #[cosmwasm_schema::cw_serde]
    pub enum LockerQuery {
    ...
    let locker: LockerResponse = deps.querier.query_wasm_smart(
        &config.lp_locker_contract,
        &LockerQuery::Locker { locker_id },
    )?;
    ...
    let duration = locker.unlock_time.saturating_sub(locker.locked_at);
    let bonus_multiplier = calculate_bonus_multiplier(duration);

    let stake = UserStake {
        user: locker.owner.clone(),
        locker_id,
        lp_token: locker.lp_token.clone(),
        lp_amount: locker.amount,
        locked_at: locker.locked_at,
        bonus_multiplier,
    };
    USER_STAKES.save(deps.storage, locker_id, &stake)?;
    USER_STAKED_LOCKERS.save(deps.storage, (&locker.owner, locker_id), &true)?;

    // Initialize reward records for all pools of this LP token
    let pool_ids = get_pools_for_lp(deps.as_ref(), &locker.lp_token)?;
    for pool_id in pool_ids {
        let pool = update_pool(deps.storage, &env, pool_id)?;
        USER_REWARDS.save(
            deps.storage,
    ...
    TOTAL_STAKED.update(deps.storage, &locker.lp_token, |total| -> StdResult<_> {
        total
            .unwrap_or_default()
            .checked_add(locker.amount)
            .map_err(cosmwasm_std::StdError::from)
    })?;
...
pub fn migrate(
    ...
    let new_config = RewardConfig {
        ...
        lp_locker_contract: old_config.lp_locker_contract,
        paused: old_config.paused,
        claim_interval: old_config.claim_interval,
        next_pool_id: old_config.next_pool_id,
        referral_commission_bps: 500, // Default 5%
        batch_limit: 20,              // Default 20
    };
